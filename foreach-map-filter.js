...
function showFirstAndLast(arr){
   ...
   arr.forEach(function(val) {
    newFirstAndLast.push(val[0] + val[val.length - 1]) 
   });

   return newFirstAndLast;
}
...
function vowelCount(str){
   ...
   splitArr.forEach(function(val) {
    ...
    if(vowels.indexOf(lowerCasedVal) !== -1) {
        if(obj[lowerCasedVal]) {
            obj[lowerCasedVal]++;
        }
        else {
            obj[lowerCasedVal] = 1;
...
of the vowels (both uppercased and lowercased) removed. Every character in the new string should be lowercased.
...
function removeVowels(str) {
    ...
    const splitLowerCasedStr = lowerCasedStr.split("");

    return splitLowerCasedStr.filter(function(value){
...
with all of the odd numbers doubled (HINT - you can use map and filter to double and then filter the odd numbers).
